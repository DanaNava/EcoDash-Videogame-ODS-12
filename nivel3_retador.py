...
def run_level3_retador(idioma_actual, volumen_actual, personaje=0):
    pygame.init()
    pygame.mixer.init()
    ...
    # -----------------------------
    fondo = pygame.image.load(os.path.join(BASE_DIR, "assets_PI", "diseyo_nivel", "nivel 3", "Fondo level3.png")).convert_alpha()

    #Charcos
    charco = pygame.image.load(os.path.join(BASE_DIR, "assets_PI", "diseyo_nivel", "nivel 3", "pega.png")).convert_alpha()
    jugo = pygame.image.load(os.path.join(BASE_DIR, "assets_PI", "diseyo_nivel", "nivel 3", "jugo.png")).convert_alpha()
    
    #Capas
    casillero= pygame.image.load(os.path.join(BASE_DIR, "assets_PI", "diseyo_nivel", "nivel 3", "casilla.png"))
    pared= pygame.image.load(os.path.join(BASE_DIR, "assets_PI", "diseyo_nivel", "nivel 3", "Sprite-0004.png"))
    ...
    # -----------------------------
    # ANIMACIONES DEL PERSONAJE MUJER (CON RUTAS CORREGIDAS)
    # -----------------------------
    ...
    frames_muerte_delante_mujer = frames_muerte_derecha_mujer
    frames_muerte_detras_mujer = frames_muerte_derecha_mujer 

    frames_caminar_delante_mujer = [
    ...
    frames_caminar_izquierda_mujer = [
    ...
    # CORRECCIÓN: Bloquear completamente el movimiento durante la muerte
        if not animando_muerte and not tiempo_fin_animacion:
            ...
            accion_realizada = False  # al inicio del frame

            # 1) TIRAR BASURA si traes algo en la mano
            ...
            # 2) RECOGER BASURA solo si NO traes nada y no hiciste ya algo
            if pressed_enter and not accion_realizada:
                basura_cercana = False
                for obj in basura[:]:
                    if hitbox.inflate(12, 12).colliderect(obj["rect"]):
                        basura_cercana = True
